node {
  ...
  //  }
   stage('test-db') {
     ...
     def mongodb = docker.image('mongo:latest').run(" --name db -e MONGO_INITDB_DATABASE=nodedb -p 27017:27017 -v /mongo-init.js:/docker-entrypoint-initdb.d/mongo-init.js:ro ")
